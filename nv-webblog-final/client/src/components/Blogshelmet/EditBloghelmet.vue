<template>
  <div>
    <h1>แก้ไข ข้อมูลหมวกกันน็อค</h1>
    <form v-on:submit.prevent="editBloghelmet">
      ...
      <p>
        <button type="submit">แก้ไข หมวกกันน็อค</button>
        <button v-on:click="navigateTo('/blogshelmet')">กลับ</button>
      </p>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    async editBloghelmet() {
      try {
        await BlogshelmetService.put(this.bloghelmet);
        this.$router.push({
          name: "blogshelmet"
        });
      } catch (err) {
  ...
  async created() {
    this.reset();
    try {
      let bloghelmetId = this.$route.params.bloghelmetId;
      this.bloghelmet = (await BlogshelmetService.show(bloghelmetId)).data;
    } catch (error) {
      console.log(error);
    }
  },
